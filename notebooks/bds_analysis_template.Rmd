...
The goal of this notebook is to: - Document each dataset with structure, size, and contents - Provide variable-level metadata (codebooks) - Generate summary statistics - Explore usage and reproducibility templates for future research
...
df_variables %>%
  bind_rows(.id = "key") %>%
  relocate(key, variable, description) %>%
  mutate(value = TRUE) %>%
  pivot_wider(id_cols = key, names_from = variable) %>%
  select(key, metro:esizecoarse)

```

```{r}
df_variables <- list()
for (key in pubdata::ls("bds")) {
  df_variables[[key]] <- tibble(
    variable = names(pubdata::meta("bds", key, print = FALSE)$schema),
    description = pubdata::meta("bds", key, print = FALSE)$schema %>% map_chr(~ .x$desc)
  )
}
df_variables %>%
  bind_rows(.id = "key") %>%
...
# Step 1: Get variable metadata for each dataset
df_variables <- map(pubdata::ls("bds"), function(key) {
  meta <- pubdata::meta("bds", key, print = FALSE)$schema
  tibble(
    variable = names(meta),
    description = map_chr(meta, ~ .x$desc)
  )
}) %>%
  set_names(pubdata::ls("bds"))  # name the list by dataset

# Step 2: Combine into a long dataframe
df_all_vars <- bind_rows(df_variables, .id = "key")

# Step 3: (Optional) Make a wide matrix: which dataset has which variable
df_wide <- df_all_vars %>%
  mutate(value = TRUE) %>%
  pivot_wider(id_cols = key, names_from = variable, values_from = value, values_fill = FALSE)
...
```{r}
df_tree <- df_all_vars %>%
  mutate(Collection = "bds") %>%
  select(Collection, Dataset = key, Variable = variable, Description = description)
df_tree
```
...
collapsibleTree(
  df_tree,
  hierarchy = c("Collection", "Dataset", "Variable"),
  tooltip = TRUE,
  collapsed = FALSE
)
#
