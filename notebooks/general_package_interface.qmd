...
`collection`: print the available keys in a specific collection.
For example, keys in the `"naics"` collection:
...
`pattern`: print all the keys that match specific regex pattern within the data collection.
For example, to print the keys with regex pattern `"2022"` in the `"naics"` collection:
...
Download and read the dataset in specific data collection (arguments`collection` and `key` must be included).
