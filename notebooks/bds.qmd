...
summary_tbl <- map_dfr(keys, function(k) {
  m <- suppressMessages(meta("bds", "raw_2022_met"))
...
codebook <- map_dfr(keys, function(k) {
  m <-  suppressMessages({
    capture.output(meta_obj <- meta(collection, k))
    meta_obj
  })
...
meta_long <- purrr::map_dfr(keys, function(k) {
  ...
  m <- suppressMessages({
    tmp <- capture.output(res <- meta(collection, k))
    res
  })
  schema <- m$schema
  if (m$type == "table") {
...
specific_cols <- names(true_counts[true_counts == 1])          # columns with all FALSE
